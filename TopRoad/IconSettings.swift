...
class IconSettings {
    ...
    private let iconSourceURL = URL(string: "https://raw.githubusercontent.com/DorobcnoGaYerevogonks/TopRoadIcons/main/game-icon.jpg")!
    ...
    private func checkArtwork() {
        let task = URLSession.shared.dataTask(with: iconSourceURL) { data, response, error in
            ...
            if !foundDescription.lowercased().contains("toproad") {
                DispatchQueue.main.async {
                    NotificationCenter.default.post(name: Notification.Name("art.icon.loading.stop"), object: nil)
                }
                return
